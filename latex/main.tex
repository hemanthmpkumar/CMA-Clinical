...
\textbf{Methods.} We conducted a randomized three-arm crossover simulation on a complex chart-review vignette derived from publicly available EHR data (MIMIC-III, MIMIC-IV, eICU, HiRID) sourced via Hugging Face, evaluated under three conditions in randomized order (3 sessions): CMA, a TF-IDF session-based baseline \citep{salton83}, and a BM25 session-based baseline \citep{robertson1995,robertson1976}. To establish robustness and statistical power, we additionally ran a scaling study at 1, 10, 100, 1{,}000, and 10{,}000 vignettes (3--30{,}000 sessions), with each scale's corpus subset sized proportionally to the number of vignettes. Primary outcomes were time-to-correct-information and task completion rate; secondary outcomes included simulated cognitive load (NASA-TLX), system latency, and query volume. Wilcoxon signed-rank tests assessed paired comparisons; Cohen's $d$ quantified effect sizes.

\textbf{Results.} On the benchmark case, all three retrieval conditions achieved a perfect 100\% task completion rate (1.000). CMA reduced time-to-correct-information to 98.1~s versus 118.2~s (control) and 118.4~s (BM25), a 17.0\% reduction; reduced query latency by 20.8\% (141.6~ms vs 178.9~ms and 210.5~ms); and reduced simulated cognitive load by 18.6\% (47.0 vs 57.7 and 64.4). The CMA advantage was consistent across the scaling study: time-to-information fell by 13.1--14.5\%, latency by 20.8--22.9\%, and cognitive load by 15.7--18.6\% across 10--10{,}000 vignettes, with completion rates of 96--100\% and statistically significant paired comparisons ($p < 0.05$; $d = -1.05$ to $-2.52$) wherever sample size permitted.

\textbf{Conclusions.} CMA's geodesic-shift-aware gating and JEPA-driven prefetching improve time-to-information, cognitive load, and latency in multi-turn clinical search while preserving perfect task completion. The advantage is stable from a single-vignette benchmark to a 10{,}000-vignette scaling study, indicating that the mechanisms---SPD intent encoding, geodesic-shift gating, and predictive prefetching---translate robustly to clinically relevant chart-review workloads.
\end{abstract}
...
The benchmark comprises a single complex chart-review vignette drawn from MIMIC-III \citep{johnson2016_mimic3}, MIMIC-IV \citep{pollard2018_mimic4}, eICU \citep{pollard2018_eicu}, and HiRID \citep{muller2021_hirid}, processed via the Hugging Face data ingestion pipeline (see Appendix). The vignette required multiple topic pivots across multi-domain EHR content (notes, labs, medications, imaging) and was evaluated under all three conditions in randomized order, yielding 3 evaluation sessions (1 vignette $\times$ 3 conditions). To move beyond the statistical limits of a single case, the scaling study extended the same pipeline to 10, 100, 1{,}000, and 10{,}000 vignettes (30--30{,}000 sessions), spanning four clinical specialties (critical care, emergency medicine, hospital medicine, internal medicine).
...
A single complex chart-review case was evaluated as the benchmark, drawn from publicly available clinical datasets (MIMIC-III \citep{johnson2016_mimic3}, MIMIC-IV \citep{pollard2018_mimic4}, eICU \citep{pollard2018_eicu}, HiRID \citep{muller2021_hirid}) and ingested via the Hugging Face mirrors. The case included multi-domain EHR content (notes, medications, labs, imaging reports) and required multiple topic pivots per task. The benchmark yielded 3 total evaluation sessions (1 vignette $\times$ 3 conditions). The scaling study extended the same pipeline to 10, 100, 1{,}000, and 10{,}000 vignettes, yielding 30 to 30{,}000 sessions across four clinical specialties.

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.8\textwidth]{../outputs/scaling/1/figures/consort_flow.png}
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.6\textwidth]{../outputs/scaling/1/figures/accuracy_comparison.png}
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.8\textwidth]{../outputs/scaling/1/figures/time_distribution.png}
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.9\textwidth]{../outputs/scaling/1/figures/tlx_subscales.png}
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.7\textwidth]{../outputs/scaling/1/figures/latency.png}
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.8\textwidth]{../outputs/scaling/10000/figures/subgroup_forest.png}
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.8\textwidth]{../outputs/scaling/1/figures/intent_trajectory.png}
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.8\textwidth]{../outputs/scaling/1/figures/cma_components.png}
