...
\begin{corollary}
\label{cor:closure}
Under the conditions of Theorem~\ref{thm:contam}, the GSI gate guarantees that after an abrupt pivot the contamination ratio never exceeds $\tfrac12 e^{-\alpha d_g(S_{t-1},S_t)} + \delta_f/\delta_0$, where $\delta_f = \|P_\xi e_t\|$ is the intrinsic leakage of a fresh query. Stale context therefore cannot dominate the retrieval embedding after a pivot; it decays geometrically with the magnitude of the diagnostic jump.\end{corollary}
...
\section{Study Design and Evaluation Protocol}
\label{sec:design}
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.75\textwidth]{../outputs/scaling/10/figures/consort_flow.png}
...
\begin{figure}[htbp]
  ...
  \begin{subfigure}[b]{0.32\textwidth}
    ...
    \caption{Scale 1000}
  \end{subfigure}
  \caption{Ablation study results across scales.}
  \label{fig:ablation}
\end{figure}
